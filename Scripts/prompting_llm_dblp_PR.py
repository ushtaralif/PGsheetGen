...
def main():
    ...
    ap.add_argument("--reload_every", type=int, default=3, help="Reload model after every N chunks.")
    ...
    for model_name, model_info in models.items():
        ...
        for i in runs:
            ...
            master_summary_output_file = os.path.join(out_dir, f"{model_name}_Master_Summary_Output_dblp_{i}PR.txt")
            llm_output_file = os.path.join(out_dir, f"{model_name}_LLM_Outputs_dblp_{i}PR.txt")
            ...
            for chunk_i, ch in enumerate(chunks):
                if chunk_i == 0 or (chunk_i % args.reload_every == 0):
                    if model is not None:
                        print(f"\n[reload] Reinitializing {model_name} before chunk {chunk_i + 1}")
