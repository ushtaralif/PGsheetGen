...
def main():
    ...
    ap.add_argument("--reload_every", type=int, default=3, help="Group size for hierarchical summarization.")
    ...
    for model_name, model_info in models.items():
        ...
        for i in runs:
            ...
            master_summary_output_file = os.path.join(
                out_dir, f"{model_name}_Master_Summary_Output_dblp_{i}.txt"
            )
            llm_output_file = os.path.join(
                out_dir, f"{model_name}_LLM_Outputs_dblp_{i}.txt"
            )
            ...
            def hierarchical_summarization(
                ...
                while len(current_items) > 1:
                    level += 1
                    print(f"\n=== {model_name} | Run {run_id} | LEVEL {level} | items={len(current_items)} ===")

                    new_items = []
                    num_groups = (len(current_items) + group_size - 1) // group_size

                    for g in range(num_groups):
                        group = current_items[g * group_size:(g + 1) * group_size]
                        print(f"[{model_name}] Run {run_id} | Level {level} | Group {g+1}/{num_groups} | size={len(group)}")
                        ...
                        prev_summary_file = master_summary_file
                        result = None
                        ...
                        for step_i, item in enumerate(group):
                            ...
                            with open(llm_output_file, "a", encoding="utf-8") as f:
                                f.write(
                                    f"\n=== {model_name} | Run {run_id} | Level {level} | Group {g+1} | Step {step_i+1} ===\n"
                                )
                                f.write(result)
                                f.write("\n")

                            tmp_summary_path = os.path.join(
                                out_dir,
                            ...
                            prev_summary_file = tmp_summary_path
                            time.sleep(sleep)
            ...
            final_summary = hierarchical_summarization(
                ...
                master_summary_file=master_summary_file,
                out_dir=out_dir,
                run_id=i,
                group_size=args.reload_every,
                sleep=args.sleep,
            ...
            final_path = os.path.join(out_dir, f"{model_name}_FINAL_run{i}.txt")
    ...
    print(f"\n Outputs has been saved at: {out_dir}")
